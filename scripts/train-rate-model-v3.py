#!/usr/bin/env python3
"""
Rate model v3 — adds Binance premium-index features on top of v2.

v2 features (24) + 4 basis features:
  basis_cur, basis_z30, basis_mom1, basis_trend

Basis = premiumIndex close (≈ (mark - index) / index). Theory: when basis
runs rich, funding rises next; when basis goes inverted, funding falls.
Historical coverage: premiumIndex klines start 2019-12-23, so we lose
about 100 early samples relative to v2 but retain >1400 samples/duration.
"""

import json, os, pickle, time, datetime
...
def fetch_premium_index_daily(symbol: str) -> dict:
    """Daily premium-index close (close = (mark-index)/index-ish)."""
    cache = CACHE_DIR / f"premium_{symbol}.pkl"
    if _cache_ok(cache):
        with open(cache, "rb") as f:
            return pickle.load(f)
    ...
    out = {}
    # paginate from 2019-12-01 forward
    start_time = 1575158400000
    while True:
        params = {"symbol": symbol, "interval": "1d", "startTime": start_time, "limit": 1000}
        r = requests.get(url, params=params, timeout=30)
        r.raise_for_status()
        batch = r.json()
        if not batch:
        ...
        if len(batch) < 1000:
            break
        start_time = int(batch[-1][0]) + 86_400_000
        time.sleep(0.15)
...
def build_features(rates, btc_r, prices, fng, basis, i, market, market_list):
    if i < 30:
        return None
    cur = rates[i]
    ...
    # ── Basis (premium index) features ──
    b_cur = basis[i] if np.isfinite(basis[i]) else 0.0
    b_w30 = basis[max(0, i - 30):i]
    b_w30 = b_w30[np.isfinite(b_w30)] if len(b_w30) else b_w30
    if len(b_w30) >= 5:
        b_ma7  = float(np.mean(basis[max(0, i - 7):i][np.isfinite(basis[max(0, i - 7):i])])) if np.any(np.isfinite(basis[max(0, i - 7):i])) else 0.0
        b_ma30 = float(np.mean(b_w30))
        b_std30 = float(np.std(b_w30)) + 1e-9
        b_z30 = (b_cur - b_ma30) / b_std30
        b_prev = basis[i - 1] if np.isfinite(basis[i - 1]) else b_cur
        b_mom1 = b_cur - b_prev
        b_trend = (b_ma7 - b_ma30) / (abs(b_ma30) + 1e-9)
    else:
        b_z30 = 0.0
        b_mom1 = 0.0
        b_trend = 0.0
...
def align_series(rate_ts, rates, btc_ts, btc_rates, price_dict, fng_dict, basis_dict):
    ...
    basis_a  = np.array([basis_dict.get(ts, np.nan) for ts in rate_ts])

    for arr in [btc_a, prices_a, fng_a, basis_a]:
        for j in range(1, len(arr)):
            if not np.isfinite(arr[j]):
                arr[j] = arr[j-1]

    return rates, btc_a, prices_a, fng_a, basis_a
...
def main():
    ...
    print("Fetching Binance premium-index klines...")
    basis_data = {}
    for market, symbol in SYMBOLS.items():
        basis_data[market] = fetch_premium_index_daily(symbol)
        print(f"  {market}: {len(basis_data[market])} daily basis pts")
    ...
    for duration in DURATIONS:
        ...
        for market in market_list:
            ...
            rates_a, btc_a, prices_a, fng_a, basis_a = align_series(
                ts, rates, btc_ts, btc_rates, price_dict, fng_dict, basis_data[market]
            )
            for i in range(30, len(rates_a)):
                y = log_ratio_target(rates_a, i, duration)
                if y is None:
                    continue
                feat = build_features(rates_a, btc_a, prices_a, fng_a, basis_a, i, market, market_list)
                if feat is None:
                    continue
                X_all.append(feat)
                y_reg_all.append(y)
        ...
        onnx_path = f"app/src/lib/fundex/lgb_{duration}d_v3.onnx"
        ...
        models[str(duration)] = {
            ...
            "has_onnx":         True,
            "onnx_file":        f"lgb_{duration}d_v3.onnx",
        }
    ...
    path = "app/src/lib/fundex/rate-model-v3.json"
