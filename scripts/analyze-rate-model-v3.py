#!/usr/bin/env python3
"""
analyze-rate-model-v3.py  —  Phase 2 measurement (v3 with basis features).

Mirrors analyze-rate-model.py but uses the v3 feature set (adds 4 premium-index
features). Outputs to separate files so before/after can be diffed directly.

Outputs:
  docs/benchmarks/rate-model-analysis-v3.md
  app/public/charts/model-calibration-v3.png
  app/public/charts/model-coverage-v3.png
"""
...
# ── Feature engineering (mirror of train-rate-model-v3.py) ──────────────────
...
def build_features(rates, btc_r, prices, fng, basis, i, market, market_list):
    ...
    b_w30 = b_w30[np.isfinite(b_w30)]
    if len(b_w30) >= 5:
        b_slice7 = basis[max(0, i - 7):i]
        b_slice7 = b_slice7[np.isfinite(b_slice7)]
        b_ma7 = float(np.mean(b_slice7)) if len(b_slice7) else 0.0
...
def main():
    ...
    print("Fetching Binance premium-index klines (for basis features)...")
    basis_data = {}
    for market, symbol in SYMBOLS.items():
    ...
    v3_path = ROOT / "app" / "src" / "lib" / "fundex" / "rate-model-v3.json"
    ...
    out_md = REPORT_DIR / "rate-model-analysis-v3.md"
    write_markdown(all_results, out_md)
    print(f"\nReport → {out_md}")

    cal_png = CHART_DIR / "model-calibration-v3.png"
    plot_calibration(all_results, cal_png)
    print(f"Chart → {cal_png}")

    cov_png = CHART_DIR / "model-coverage-v3.png"
