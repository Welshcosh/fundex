...
Fundex 는 Solana 네이티브 perpetual 펀딩비를 기초자산으로 하는, 완전 온체인 고정-변동(fixed-for-floating) 이자율 스왑(IRS) 프로토콜입니다. 트레이더는 16 개 마켓(4 perp × 4 duration)에서 **Fixed Payer** 또는 **Fixed Receiver** 포지션을 잡을 수 있고, 퍼미션리스 **LP Pool** 이 양측의 net imbalance 에 대한 counterparty 역할을 수행합니다.
...
Fundex 는 프로덕션 트레이딩 베뉴가 아닌 **레퍼런스 구현**입니다. 목적은 Solana 네이티브, perp 네이티브, 오라클 없이 동작하는 funding rate swap 이 실제로 어떤 모습인지 탐구하는 것입니다.
...
- **온체인 rate source** — 온체인 perp market 계정의 `lastFundingRate` 를 직접 읽고, program-owner 검증으로 무신뢰성 보장. 오프체인 오라클·릴레이 없음.
- **Perp 네이티브 마켓 매핑** — Solana perp 마켓과 1:1 매핑 (BTC / ETH / SOL / JTO).
...
| Account | Seeds | 설명 |
|---------|-------|------|
| `RateOracle` | `[rate_oracle, perp_index]` | 온체인 perp 펀딩비의 EMA |
...
Fundex 는 **온체인 perp market 계정에서 직접** 펀딩비를 읽으며, 신뢰해야 할 오프체인 입력이 없습니다.

```
settle_funding():
  1. perp_market.owner == EXPECTED_PERP_PROGRAM_ID 검증
  ...
  3. 변환 (i128-safe):
     ...
     (lastFundingRate 는 rate 가 아니라 FUNDING_RATE_PRECISION 1e9
...
크랭크는 perp market PDA 를 계정으로 전달하고, 프로그램이 owner 를 검증한 뒤 무신뢰적으로 rate 를 읽습니다. 이 구조는 신뢰해야 할 오라클이나 오프체인 rate relay 를 제거합니다. 전체 유도 과정과 2026-04-15 포스트모템은 `docs/WHITEPAPER.md` §5, §11 참고.
...
Trade 페이지(`/trade`)는 익숙한 퍼페추얼 거래소처럼 레이아웃 — 차트와 AI 밴드가 메인 칼럼을 차지하고, 우측 3-칼럼에 오더북 · 최근 체결 · 주문 패널이 배치됩니다. 1440px+ 화면에서는 한 viewport에 다 들어갑니다 (데모 시 스크롤 불필요).

### 레이아웃

```
┌─ TradeHeader (마켓 셀렉터, duration 탭) ──────────────────────────────┐
├─ AI Rate Advisor 밴드 (가로) — 방향, confidence,                      │
│   추천 fixed rate, Claude one-line reasoning, OOS 정확도              │
├──────────────────────────────────────────┬──────┬───────┬─────────────┤
│  Rate Chart                              │ Book │ Trades│ Order Panel │
│   • Line / Candles 토글                  │      │       │   Side      │
│   • Timeframes: 1s · 15s · 1m · 5m · 1h  │ asks │ live  │   Size      │
│     · 4h · 1d                            │      │ feed  │   Summary   │
│   • OHLCV 캔들 + 4단 가격 그리드          │ mid  │       │   Est. PnL  │
│     + 거래량 바 + 시간축                  │      │       │   [Open ▶]  │
├──────────────────────────────────────────┤ bids │       │             │
...
각 타임프레임은 deterministic 가격 시리즈를 렌더링하되 마지막 포인트가 정확히 라이브 오라클 rate와 일치하도록 스케일됩니다 ("차트 끝 = 현재가" 거래소 컨벤션 유지). 라이브 타임프레임(`1s`, `15s`)은 1.2–2.7초마다 새 틱을 추가합니다.

| 타임프레임 | 데이터 소스 |
|-----------|-------------|
| `1s` · `15s` | 온체인 `FundingSettled` 이벤트 + 라이브 틱 |
| `1m` · `5m` · `1h` · `4h` · `1d` | `(심볼, 타임프레임)` 시드 기반 deterministic log-walk |

### 캔들 차트

`Candles` 모드:
- 연속성 있는 OHLCV (`open[i] = close[i-1]`)
- 타임프레임 volatility 비례 wick
- 거래량 바 (하단) — 캔들 방향과 동조 색
- 4단 가격 그리드 + 우측 가격 라벨
- 시간축 5개 라벨 (타임프레임에 따라 포맷 — sub-minute는 `HH:MM:SS`, daily는 `MM/DD`)

### 라이브 mock 데이터 (데모 전용)

`NEXT_PUBLIC_DEMO_MODE=true`일 때 3개 컴포넌트가 독립 박자로 움직입니다:

| 컴포넌트 | 주기 | 변화 내용 |
|---------|------|----------|
| Recent Trades | 행당 1.5–3.3s | 새 행이 상단에 fade-in, 18개 윈도우 회전 |
| Order book | 2.2–3.7s | 행 사이즈 jitter (×0.55–1.55); rate는 `fixedRate`에 고정 |
| 차트 라이브 틱 | 1.2–2.7s | 우측 끝에 새 포인트 추가 (1s/15s만) |

모두 `(market.symbol, …)` 시드 기반 PRNG 사용 — 같은 마켓은 항상 같은 시작 backlog로 안정적 + 마켓 변경 시 새로 시드.
...
| Rate source | Solana 네이티브 perp 펀딩비 — 온체인 직접 읽기 |
...
### 6. 데모 모드 (priority fee + processed commitment)

데모 영상 촬영 또는 빠른 사용자 플로우를 위해 `NEXT_PUBLIC_DEMO_MODE=true`로 3가지 트랜잭션 가속 + 시각 liveness를 한 번에 활성화:
...
| 항목 | OFF (기본) | ON (demo) |
|------|------------|-----------|
| Priority fee | 없음 | `setComputeUnitLimit(400_000)` + `setComputeUnitPrice(100_000 µlamports)` ≈ tx당 0.00004 SOL |
| 지갑 commitment | `confirmed` (~1.5s) | `processed` (~400ms) |
| Preflight | `skipPreflight: false` | `skipPreflight: true` |
| 시각 표시 | 숨김 | navbar 우상단 주황 `DEMO` 배지 |
| Recent Trades 스트림 | 시드 backlog만 | 1.5–3.3s마다 라이브 |
| Order book jitter | 정적 | 2.2–3.7s마다 사이즈 변동 |
| 차트 라이브 틱 | 정적 | `1s`/`15s`에서 1.2–2.7s마다 새 틱 |
| 24h Volume 셀 | 숨김 | `MarketStatsBar`에 표시 |

env var가 unset이면 모든 knob이 기본값으로 복귀 (간단한 null/empty-array 가드) — prod이 demo-only 완화를 silent하게 상속하지 않습니다. navbar 주황 `DEMO` 배지가 안전 표시 인디케이터.

**Production 배포**: Vercel env에 `NEXT_PUBLIC_DEMO_MODE=true` 추가 (Production + Preview, `--no-sensitive` flag) — 빌드 타임에 사이트가 인라인.
